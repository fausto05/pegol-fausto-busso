...
function checkBucketCollision() {
  ...
  if (circleRectCollision(ball, bucket) && ball.vy > 0) {
    ...
    if (!recoveredShotInCurrentTurn) {
      ...
      stateMsg.textContent = "Recuperaste 1 tiro en el bucket.";
    }
  }
}
...
function finishTurn() {
  ...
  if (targetsLeft === 0) {
    gameState = "win";
    stateMsg.textContent = "Ganaste: no quedan pegs objetivo.";
  } else if (shots <= 0) {
    gameState = "lose";
    stateMsg.textContent = "Perdiste: te quedaste sin tiros.";
  } else {
...
function drawBackground() {
  ...
  ctx.font = "16px Arial";
  ctx.fillText("Nivel 1 - Pegol (Etapa base)", 20, 28);
}
...
function drawShooterAndAim() {
  ...
  ctx.lineTo(SHOOTER.x + Math.cos(clampedAim) * 34, SHOOTER.y + Math.sin(clampedAim) * 34);
  ctx.stroke();
...
function drawBucket() {
  ...
  ctx.fillRect(bucket.x - bucket.w / 2, bucket.y - bucket.h / 2, bucket.w, bucket.h);
}
